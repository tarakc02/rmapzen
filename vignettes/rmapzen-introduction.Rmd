...
`rmapzen` is a client for any implementation of the Mapzen API. Though Mapzen itself has [gone out of business](https://mapzen.com/blog/shutdown/), `rmapzen` can be set up to work with any provider who hosts Mapzen's open-source software, including [geocode.earth](https://geocode.earth/), [Nextzen](https://www.nextzen.org/), and [NYC GeoSearch from NYC Planning Labs](https://geosearch.planninglabs.nyc/). For more information, see [https://mapzen.com/documentation/](https://mapzen.com/documentation/). The project is [available on github](https://github.com/tarakc02/rmapzen) as well as [CRAN](https://cran.r-project.org/package=rmapzen). 

`rmapzen` provides access to the following Mapzen API services:

- [Search](https://mapzen.com/documentation/search/): Structured and unstructured search and geocoding, reverse geocoding, and autocomplete. 
- [Vector Tiles](https://mapzen.com/documentation/vector-tiles/): Basemap vector data.
- [Isochrone](https://mapzen.com/documentation/mobility/isochrone/api-reference/): Calculation of areas reachable from a specified locations.
...
`rmapzen` provides an interface to Mapzen's [vector tiles service](https://mapzen.com/documentation/vector-tiles/). Tile requests can be specified using the x, y, zoom coordinates of the tile service, as well as with a lat/long bounding box. Multiple tiles are stitched together and returned as an object of class `mz_vector_tiles`. See `?mz_vector_tiles`. The sample data set `ca_tiles` contains zoomed out vector tile data for all of California as well as parts of neighboring states.
